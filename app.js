...
veryOldInventors = inventors.filter((inventor, index, array) => {
...
inventorNames = inventors.map((inventor, index, array) => {
  let result = { first: inventor.first, last: inventor.last }
  return result
})
...
isAdultPresent = devs.some((dev) => {
  let age = Date().getFullYear - dev.year
...
idx = comments.findIndex((comment, index, array) => {
